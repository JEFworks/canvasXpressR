...
#'
#' @param mat Matrix
#' @param groups Factor grouping. Should match columns of matrix.
#'
#' @examples
#' data(mtcars)
#' mat <- t(mtcars)
#' groups <- mtcars$cyl
#' write(cXboxplot(mat, groups), file="boxplot.html")
#'
#' @export
#'
...
#' Writes HTML page with interactive scatterplot given a set of X and Y values
#'
#' @param df 2xN dataframe or matrix where first column is X values and second is Y values
#' @param groups Factor groupings for each value
#'
#' @examples
#' data(mtcars)
#' basePCA <- prcomp(mtcars)
#' groups <- paste0("cyl:", mtcars$cyl)
#' df <- basePCA$x[,1:2]
#' write(cXscatter(df, groups), file="scatter.html")
#'
#' @export
#'
cXscatter <- function(df, groups) {
  dataString <- writeDataString(df)
  groupsString <- paste0("{ 'groups' : [", paste0(paste0('"', groups, '"'), collapse=", "), "] }")
  paste0('<html>
         <head>
         <link rel="stylesheet" href="http://www.canvasxpress.org/css/canvasXpress.css" type="text/css"/>
